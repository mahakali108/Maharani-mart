...
/**
 * Parses a pasted order list into (quantity, term) pairs. Pure, exported for
 * tests. Accepted shapes per line:
 *   `2 x parle-g 50g`   `2x parle-g`   `parle-g 50g x 24`   `parle-g`
 * A bare term gets quantity null (the UI fills the pack MOQ).
 */
export function parseBulkOrderLines(raw: string, maxLines = 20): { quantity: number | null; term: string; line: string }[] {
  const results: { quantity: number | null; term: string; line: string }[] = [];
