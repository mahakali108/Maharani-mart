...
import { Check, ChevronRight, Loader2, PackageCheck, ShieldCheck, ShoppingCart, Zap } from 'lucide-react';
import { addToCartAction, buyNowAction } from '@/lib/retailer/cart-actions';
...
export function PackSelector({ packs, gstPercent }: { packs: Pack[]; gstPercent: number }) {
  ...
  function handleBuyNow() {
    ...
    setError(null);
    startTransition(async () => {
      const result = await buyNowAction(selectedPack.id, quantity);
      if (result && 'error' in result) setError(result.error ?? 'Could not start checkout.');
      else router.push('/retailer/checkout');
    });
  }
  ...
  const discount = calcDiscountPercent(selectedPack.mrp, selectedPack.effectivePrice);
  const saveEach = calcSavings(selectedPack.mrp, selectedPack.effectivePrice);
  const saveTotal = calcSavings(selectedPack.mrp, selectedPack.effectivePrice, quantity);
      ...
      <div className="space-y-5 p-4 sm:p-5">
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
          {packs.map((pack) => {
            ...
            return (
              ...
              >
                {active ? (
                  <span className="absolute right-2 top-2 flex h-4 w-4 items-center justify-center rounded-full bg-primary-600 text-white">
                    <Check className="h-2.5 w-2.5" />
                  </span>
                ) : null}
                <p className={`pr-5 text-xs font-bold ${active ? 'text-primary-800' : 'text-slate-800'}`}>{pack.pack_name}</p>
                <p className="mt-1 text-[10px] text-slate-500">
                  {pack.units_per_case} unit(s) per pack · MOQ {pack.moq}
                </p>
                <p className="mt-2 text-sm font-bold text-slate-950">{formatInr(pack.effectivePrice)}</p>
              </button>
            );
          })}
        </div>
        ...
        <div className="rounded-xl bg-slate-50 p-3.5">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <div className="mt-1 flex flex-wrap items-baseline gap-2">
                <p className="text-2xl font-bold tracking-tight text-slate-950">{formatInr(selectedPack.effectivePrice)}</p>
                {selectedPack.mrp && selectedPack.mrp > selectedPack.effectivePrice ? (
                  <p className="text-xs text-slate-400 line-through">MRP {formatInr(selectedPack.mrp)}</p>
                ) : null}
                {discount > 0 ? (
                  <span className="rounded-md bg-emerald-100 px-1.5 py-0.5 text-[9px] font-bold text-emerald-700">{discount}% off</span>
                ) : null}
              </div>
              {saveEach > 0 ? (
                <p className="mt-1 text-[11px] font-bold text-emerald-700">
                  You save {formatInr(saveEach)} / pack
                  {saveTotal > saveEach ? ` · ${formatInr(saveTotal)} on this qty` : ''}
                </p>
              ) : null}
            </div>
          ...
          <div className="mt-3 flex items-center justify-between border-t border-slate-200 pt-2 text-[10px] text-slate-500">
            <span>
              GST {gstPercent}%: {formatInr(gstAmount)} / pack
            </span>
            <span className="font-semibold text-slate-700">Landed {formatInr(priceWithGst)}</span>
          </div>
        </div>

        <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
          <div className="sm:w-36">
            <label htmlFor="pack-quantity" className="mb-1.5 block text-[10px] font-bold uppercase tracking-wider text-slate-500">
              Quantity
            </label>
            <QtyStepper id="pack-quantity" value={quantity} min={selectedPack.moq} onChange={setQuantity} label="Pack quantity" />
          ...
          >
            ...
            {isPending ? 'Adding…' : added ? 'Added to cart' : `Add to cart · ${formatInr(orderTotal)}`}
          </button>
        </div>
        ...
        >
          <Zap className="h-4 w-4 text-amber-300" /> Buy now
        </button>

        {error ? (
          <div role="alert" className="rounded-xl border border-primary-200 bg-primary-50 px-3 py-2.5 text-xs font-medium text-primary-700">
            {error}
          </div>
        ) : null}
