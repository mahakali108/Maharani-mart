...
| `0046_smoke_fixture.sql` | 4 | Private, API-invisible `smoke_fixture` schema + single-row `smoke_fixture.smoke_personas` fixture table (RLS on, SELECT granted to `authenticated`) that `supabase/smoke-test.sql` writes and rolls back. Ships empty — no data. Replaces the session-scoped TEMP fixture that made the smoke test fail with `42P01: relation "smoke_personas" does not exist` whenever a run spanned sessions (pooled SQL editor, partial run). |
